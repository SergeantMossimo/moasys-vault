...
| Short runtime                                   | The file's runtime is at or below `min_duration_minutes` — usually a truncated or failed encode. Expect legitimate hits on short films, TV specials and stand-up sets; list those under `movies:` in `ignored/<drive>/movies.yaml`. Off by default — `warn_tmdb_runtime_mismatch` is the precise version |
