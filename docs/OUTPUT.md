...
| Probe failed                                    | ffprobe could not read the file. It is excluded from probe output and has no quality data. A file ffprobe rejects is usually unplayable in Plex too — verify it plays and re-copy it if not. Has no toggle (like `permission_denied`); silence per path via `ignored/<drive>/<type>.yaml` |
