...
- **v0.8** — Audiobook name checks: series and author spelling drift, capitalization, HTML entities, and mixed punctuation across books; embedded tags compared with folders; `npm run validate:audiobooks` cross-checks titles and authors against Open Library (no API key). `.secrets.json` blocks became per-integration.
